...
let init_round_state hand1 hand2 curr_state big_blind_string player_money_int bot_money_int turn_string= {
  river = curr_state.river;
  pot = curr_state.pot;
  player_hand = hand1;
  bot_hand = hand2;
...
let find_highest_card_value cards =
  ...
  for i = 0 to List.length cards do
    if
...
let find_winner (bot_hand : Bot.hand) (player_hand : Bot.hand) river =
  let allBotCards =
    bot_hand.card1 :: bot_hand.card2 :: river
  in
  let allPlayerCards =
    player_hand.card1 :: player_hand.card2
    :: river
  in
